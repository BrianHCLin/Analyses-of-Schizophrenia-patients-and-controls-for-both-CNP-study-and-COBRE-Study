...
COBRE_logi_subset <- COBRE_RF_subset[,c("Subject_Type",names(COBRE_RF_subset[,c(1,137:150)]), subset1)] %>%
  Standarize()

COBRE_test <- COBRE_logi_subset 

COBRE_test$prediction <- predict(logi_m1, COBRE_test, type = "response")
Classify(COBRE_test, COBRE_test$prediction,"Subject_Type", 0.425 )


#Fit CNP data into COBRE model
CNP_logi_subset <- CNP_RF_subset[,c("Subject_Type",names(CNP_RF_subset[,c(1,137:150)]), subset2)] %>%
  Standarize()

CNP_test <- CNP_logi_subset

CNP_test$prediction <- predict(logi_m2, CNP_test, type = "response")
Classify(CNP_test, CNP_test$prediction,"Subject_Type", cut_off = 0.2 )
